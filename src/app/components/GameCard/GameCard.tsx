...
export default function GameCard({ title, items, className = '' }: GameCardProps) {
    ...
    return (
        ...
        >
            <h2 className="card-title text-lg sm:text-xl break-words text-left">{title}</h2>
            <ul className="card-list">
                {items.map((item, index) => (
                    <li key={index} className="card-list-item flex items-start text-left space-x-2 break-words">
